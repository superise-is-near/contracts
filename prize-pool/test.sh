...
## 给bob一些存储费用
#near call $SKYWARD_TOKEN_ID storage_deposit '' --accountId bob.$ACCOUNT_ID --amount 0.00125
## 给合约一些存储费用
#near call $SKYWARD_TOKEN_ID storage_deposit '' --accountId $PRIZE_POOL --amount 0.00125
## 给bob打钱
#near call $SKYWARD_TOKEN_ID ft_transfer '{"receiver_id": "'bob.$ACCOUNT_ID'", "amount": "100"}' --accountId $ACCOUNT_ID --amount 0.000000000000000000000001

## 将prizepool 的合约发布，测试账号：dev-1634785098585-69379278107354
...
## 创建一个合约小号
#near create-account $PRIZE_POOL --masterAccount $ACCOUNT_ID --initialBalance 10
## 重新部署合约
#near deploy $PRIZE_POOL out/prize-pool.wasm #new '{}'
